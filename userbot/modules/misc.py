...
async def sleepybot(time):
    """ .sleep komandası NeonUserBot'u yatızdırar :) """
...
async def creator(e):
    await e.edit("<b>N Σ O N UserBot @TheOksigen və @Esebj tərəfindən yaradılmışdır.</b>", parse_mode="HTML")


@register(outgoing=True, pattern="^.readme$")
async def reedme(e):
    await e.edit("https://github.com/TheOksigen/2/blob/master/README.md")
...
async def repo(event):
    await event.edit("[N Σ O N UserBot](https://github.com/TheOksigen/2/)")
...
CmdHelp('misc').add_command(
    ...
    'sleep', '<vaxt>', 'NeonUserBot\'u yazdığınız saniyə qədər yatızdırar', 'sleep 20'
).add_command(
    'shutdown', None, 'Botu Söndürər.'
).add_command(
    'repo', None, 'NeonUserBot\'un GitHub reposunun linki.'
).add_command(
    'readme', None, 'NeonUserBot\'un GitHub\'dakı README.md faylının linki.'
).add_command(
